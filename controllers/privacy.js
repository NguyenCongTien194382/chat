...
export const UnblockMessage = async (req, res, next) => {
    try {
        ...
        if (req.body.userId && req.body.userBlocked) {
            const userId = Number(req.body.userId)
            let userBlocked = []
            if (req.body.userBlocked.includes('[')) {
                userBlocked = req.body.userBlocked.replace('[', '').replace(']', '').split(',')
                userBlocked = userBlocked.map(user => Number(user))
            }
            else {
                userBlocked.push(Number(req.body.userBlocked))
            }

            const result = await Privacy.updateOne({ userId: userId }, { $pull: { blockMessage: { $in: userBlocked } } })
        ...
        if (req.body.userId && req.body.type) {
            if(req.body.type == 1){
                ...
                if (privacy) {
                    for (let i = 0; i < privacy.blockMessage.length; i++) {
                        const user = await User.findOne({ _id: privacy.blockMessage[i] }, { userName: 1, avatarUser: 1, id365: 1, type365: 1 })
                        if (user.avatarUser !== '') {
            ...
            if(req.body.type == 2){
                ...
                for (let i = 0; i < privacy.length; i++) {
                        ...
                        const user = await User.findOne({ _id: privacy[i].userId }, { userName: 1, avatarUser: 1, id365: 1, type365: 1 })
                        if (user.avatarUser !== '') {
                            user.avatarUser = `${urlImgHost()}avatarUser/${user._id}/${user.avatarUser}`
                        }
                        else {
...
export const UnblockPost = async (req, res, next) => {
    try {
        ...
        if (req.body.userId && req.body.userBlocked) {
            ...
            const result = await Privacy.updateOne({ userId: userId }, { $pull: { blockPost: {$in: userBlocked} } })
        ...
        if (req.body.userId) {
            ...
            if (privacy) {
                for (let i = 0; i < privacy.blockPost.length; i++) {
                    const user = await User.findOne({ _id: privacy.blockPost[i] }, { userName: 1, avatarUser: 1 })
                    if (user.avatarUser !== '') {
                        user.avatarUser = `${urlImgHost()}avatarUser/${user._id}/${user.avatarUser}`
                    }
                    else {
...
export const UnhidePost = async (req, res, next) => {
    try {
        ...
        if (req.body.userId && req.body.userBlocked) {
            ...
            const result = await Privacy.updateOne({ userId: userId }, { $pull: { hidePost: {$in: userBlocked} } })
        ...
        if (req.body.userId) {
            ...
            if (privacy) {
                for (let i = 0; i < privacy.hidePost.length; i++) {
                    const user = await User.findOne({ _id: privacy.hidePost[i] }, { userName: 1, avatarUser: 1 })
                    if (user.avatarUser !== '') {
                        user.avatarUser = `${urlImgHost()}avatarUser/${user._id}/${user.avatarUser}`
                    }
                    else {
...
export const GetPrivacy = async (req, res, next) => {
    try {
        ...
        if (req.body.userId) {
            ...
            const user = await User.findOne({_id: userId}, {active: 1})
            if (privacy && user) {
                privacy.active = user.active
                res.json({
                    data: {
                        result: true,
...
export const ChangeSeenMessage = async (req, res, next) => {
    try {
        if (req.body.token) {
            let check = await checkToken(req.body.token);
            if (check && check.status && (check.userId == req.body.userId)) {
        ...
        const privacy = await Privacy.findOneAndUpdate({ userId: userId }, { seenMessage: status }, { new: true })
...
export const ChangeStatusOnline = async (req, res, next) => {
    try {
        if (req.body.token) {
            let check = await checkToken(req.body.token);
            if (check && check.status && (check.userId == req.body.userId)) {
        ...
        const privacy = await Privacy.findOneAndUpdate({ userId: userId }, { statusOnline: status }, { new: true })
