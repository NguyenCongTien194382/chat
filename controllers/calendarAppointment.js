...
//                 const user = await User.findOne({ _id: formData.senderId }, { userName: 1, avatarUser: 1 })
//                 result._doc.senderName = user.userName
//                 if (user.avatarUser === '') {
//                     result._doc.senderAvatar = `${urlImgHost()}avatarUser/${user._id}/${user.avatarUser}`
//                 }
//                 else {
//                     result._doc.senderAvatar = `${urlImgHost()}avatar/${user.userName[0]}_${Math.floor(Math.random() * 4) + 1}.png`
//                 }
//                 result._doc.emotion = `${urlImgHost()}ImageReminder/${formData.emotion}.png`
...
//             if (saveCalendar) {
//                 const result = { ...saveCalendar }
//                 result._doc.emotion = `${urlImgHost()}ImageReminder/${emotion}.png`
...
export const editCalendar = async (req, res, next) => {
    try {
        ...
        if (req && req.body) {
            ...
            if (saveCalendar) {
                const result = { ...saveCalendar }
                const user = await User.findOne({ _id: saveCalendar.senderId }, { userName: 1, avatarUser: 1 })
                result._doc.senderName = user.userName
                if (user.avatarUser === '') {
                    result._doc.senderAvatar = `${urlImgHost()}avatarUser/${user._id}/${user.avatarUser}`
                }
                ...
                result._doc.emotion = `${urlImgHost()}ImageReminder/${saveCalendar.emotion}.png`
        ...
        if (req && req.params) {
            ...
            if (calendar && user) {
                ...
                if (result._doc.avatarUser !== '') {
                    result._doc.avatarUser = `${urlImgHost()}avatarUser/${result._doc.senderId}/${result._doc.senderaAvatar}`
                }
                else {
                    result._doc.avatarUser = `${urlImgHost()}avatarUser/${result._doc.senderName[0]}_${Math.floor(Math.random() * 4) + 1}.png`
                }
                result._doc.time = formatDate(calendar.createTime, calendar.typeDate)
                result._doc.interval = distanceBetweenDay(calendar.createTime, Date.now())
                result._doc.emotion = `${urlImgHost()}ImageReminder/${calendar.emotion}.png`
                res.json({
                    data: {
                        result: result._doc,
                        message: 'Success'
        ...
        if (req ) {
            ...
            if (calendar) {
                for(let i = 0; i < calendar.length; i++) {
                    calendar[i].emotion = `${urlImgHost()}ImageReminder/${calendar[i].emotion}.png`
                }
                res.json({
                    data: {
                        result: calendar,
    ...
    try {
        ...
        if (req.body.conversationId) {
            ...
            if (calendar && calendar.length > 0) {
                for (let i = 0; i < calendar.length; i++) {
                    const user = await User.findOne({ _id: calendar[i].senderId }, { userName: 1, avatarUser: 1 })
                    calendar[i]._doc.senderName = user.userName
                    if (user.avatarUser !== '') {
                        calendar[i]._doc.senderAvatar = `${urlImgHost()}avatarUser/${user._id}/${user.avatarUser}`
                    }
                    else {
                        calendar[i]._doc.senderAvatar = `${urlImgHost()}avatarUser/${user.userName[0]}_${Math.floor(Math.random() * 4) + 1}.png`
                    }
                    calendar[i].emotion = `${urlImgHost()}ImageReminder/${calendar[i].emotion}.png`
