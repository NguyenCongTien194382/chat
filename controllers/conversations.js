  ...
  try {
    ...
    if (req.body && req.body.code && req.body.userId && req.body.lat && req.body.long && req.body.name
      ...
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/JoinConversationOffline",
      ...
      if (conversations) {  // lỗi của phần này đưa vào try catch 
        if (conversations.length > 0) {
          const host = await User.find({ _id: Number(conversations[0].admin) }, { _id: 0, latitude: 1, longtitude: 1 }).lean();
  ...
  try {
          ...
          memberList: {
            $map: {
              ...
              in: {
                ...
                timeLastSeener:
                  "$$member.timeLastSeener",
                lastMessageSeen:
                  "$$member.lastMessageSeen",
                deleteTime: "$$member.deleteTime",
                deleteType: "$$member.deleteType",
                favoriteMessage:
                  "$$member.favoriteMessage",
    ...
    for (const [index, con] of listCons.entries()) {
      ...
      if (listCons[index].isGroup && listMember.length === 2) {
        conversationName = users[0]
          ? users[0].conversationName
          : users[0].userName;
        avatarConversation = `${urlImgHost()}avatar/${conversationName
          .substring(0, 1)
          .toUpperCase()}_${Math.floor(Math.random() * 4) + 1}.png`;
      }
      ...
      if (listCons[index].browseMemberList.length) {
        listCons[index].browseMemberList = listCons[index].browseMemberList.map(
          (e) => {
            const memberBrowserId = e.memberBrowserId;
            const dataBrowerMem = listCons[index].listBrowse.find(
              (e) => e._id === memberBrowserId
            );
            dataBrowerMem.lastActive = date.format(
            ...
            dataBrowerMem.avatarUser = dataBrowerMem.avatarUser
              ...
              : `${urlImgHost()}avatar/${dataBrowerMem.userName
                .substring(0, 1)
                .toUpperCase()}_${Math.floor(Math.random() * 4) + 1}.png`;
            return (e = {
...
export const ChangeShareLinkOfGroup = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/ChangeShareLinkOfGroup",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
...
export const GetListUnreaderConversation = async (req, res) => {
  try {
    ...
    let lastConv = await Conversation.find({
      memberList: { $elemMatch: { memberId: userId, unReader: { $ne: 0 } } },
    }).sort({ timeLastChange: -1 }).limit(1).select("_id").lean();
...
export const GetListMemberOfGroup = async (req, res) => {
  try {
    ...
    const dataUser = await User.find({ _id: { $in: arrayId } });
    let count = 0;
...
export const ChangeNameGroup = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/ChangeNameGroup",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    const user = await User.findOne({ _id: userId }, { userName: 1 }).lean()
    if (conversationName !== oldConversationName) {
      let sendmes = await axios({
        method: "post",
...
export const ChangeBrowseMemberOfGroup = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/ChangeBrowseMemberOfGroup",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
...
export const PinMessage = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/PinMessage",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
...
export const UnPinMessage = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/UnPinMessage",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
...
export const OutGroup = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/OutGroup",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    console.log('OutGroup body', req.body)
    ...
    if (listMember.length <= 2 && existConversation.typeGroup && !existConversation.typeGroup.includes('liveChat')) {
      await Conversation.findOneAndDelete({ _id: existConversation._id })
      listMember.map(userId => {
        if (userId !== senderId) {
          socket.emit('DeleteConversation', userId, existConversation._id)
          axios({
            method: "post",
            url: "http://43.239.223.142:9000/api/V2/Notification/SendNotification_v2",
            data: {
              Title: "Thông báo giải tán nhóm",
              Message: `Nhóm ${existConversation.memberList[0].conversationName} đã được giải tán`,
              Type: "SendCandidate",
              UserId: userId,
              SenderId: senderId
            },
...
export const OutManyGroup = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/OutManyGroup",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    if (req.body && req.body.userId && req.body.arrayConvId && String(req.body.arrayConvId).includes("[")) {
      ...
      for (let i = 0; i < arrayConvId.length; i++) {
        axios({
          method: "post",
          url: "http://43.239.223.142:9000/api/conversations/OutGroup",
          data: {
            conversationId: arrayConvId[i],
...
export const AddToFavoriteConversation = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/AddToFavoriteConversation",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
...
export const HiddenConversation = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/HiddenConversation",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
...
export const DeleteConversation = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/DeleteConversation",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    const indexMember = existConversation.memberList.findIndex(
      (e) => e.memberId === senderId
    );
    const lastMess = existConversation.messageList.at(-1);
    existConversation.memberList[indexMember].messageDisplay =
...
export const ReadMessage = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/ReadMessage",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    if (existUnreadMessage && existUnreadMessage.memberList) {
      if (existUnreadMessage.messageList && existUnreadMessage.messageList.length) {
        ...
        ).catch((e) => { console.log("Error readmessage"); return false })
      }
      else {
        ...
        '$project': {
          ...
          'conversationName': {
            '$cond': [
              ...
              }, '$name2.conversationName', '$name1.userName'
            ]
          }
        }
      }, {
    ...
    for (const [index, con] of listCons.entries()) {
      ...
      if (!con.isGroup && users[0]) {
        avatarConversation = users[0].avatarUser
          ...
          : `${urlImgHost()}avatar/${users[0].userName
            .substring(0, 1)
            .toUpperCase()}_${Math.floor(Math.random() * 4) + 1}.png`;
      ...
      if (con.isGroup) {
        avatarConversation = con.avatarConversation
          ...
          : `${urlImgHost()}avatar/${con.conversationName
            .substring(0, 1)
            .toUpperCase()}_${Math.floor(Math.random() * 4) + 1}.png`;
...
export const AddBrowseMember = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/AddBrowseMember",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    if (!existConversation)
      return res
        .status(200)
        .send(createError(200, "Không thể thêm người kiểm duyệt"));
    const data = {
      result: "true",
...
export const MarkUnreader = async (req, res) => {
  try {
    ...
    if (!updatedConver)
      return res.send(createError(200, "Người dùng chưa đọc hết tin nhắn"));
    const idxMem = updatedConver.memberList.findIndex(
      (e) => e.memberId === senderId
    );
  ...
  try {
    if (req.body.token) {
      ...
      if (check && check.status && (check.userId == req.body.adminId)) {
        //, ChangeNotificationConversation")
      }
      else {
        return res.status(404).json(createError(404, "Invalid token"));
      }
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/ChangeNotificationConversation",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    const conversationId = Number(req.body.conversationId) || null;
    let notification =
      Number(req.body.notification) === 0 ? 0 : Number(req.body.notification);
...
export const RemoveConversation = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/RemoveConversation",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
...
export const AddNewConversation = async (req, res) => {
  try {
    ...
    if (memList.length < 2 || conversationName.trim() === '') {
      return res.status(400).send(createError(400, "Thong tin truyen len khong hop le"))
    }
    const listName = await User.find({ _id: memList }).select('userName').lean()
    ...
    if (!conversationName && listName.length === 2) {
      conversationName = listName.map(e => e =
        e.userName
      ).join(', ')
    }
...
export const CreateNewSecretConversation = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/CreateNewSecretConversation",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
...
export const AddNewMemberToGroup = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/AddNewMemberToGroup",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    const query = {
      _id: conversationId,
      $and: [
        { "memberList.memberId": { $nin: memberList } },
        { "memberList.memberId": senderId },
    ...
    if (!con)
      return res.send(createError(200, "Thêm thành viên vào nhóm thất bại"));
    const data = {
      result: true,
...
export const DeleteBrowse = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/DeleteBrowse",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    exCon.browseMemberList = exCon.browseMemberList.filter((e) =>
      !memberList.includes(e.memberBrowseId)
    );
    if (!exCon) {
      return res.send(
        createError(200, "Xóa thành viên duyệt vào nhóm thất bại")
      );
    }
    await exCon.save();
    const data = {
...
export const GetInfoLiveChat = async (req, res) => {
  try {
    ...
    const listCons = await User.aggregate(pipeline);
...
export const ChangeAvatarGroup = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/ChangeAvatarGroup",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
...
export const ChangeNickName = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/ChangeNickName",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
...
export const GetListConversationFavoriteOrder = async (req, res) => {
  try {
    if (req.body.token) {
      let check = await checkToken(req.body.token);
      if (check && check.status && (check.userId == req.body.userId)) {
        //, GetListConversationFavoriteOrder")
      }
      else {
        return res.status(404).json(createError(404, "Invalid token"));
      }
    ...
    let listCons = await User.aggregate([
      {
        '$match': {
          '_id': userId
        }
        ...
        '$project': {
          ...
          'conversationName': {
            '$cond': [
              ...
              }, '$name.conversationName', '$name1.userName'
            ]
          }
        }
      }, {
    ...
    for (const [index, con] of listCons.entries()) {
      ...
      if (con.isGroup) {
        avatarConversation = con.avatarConversation ? `${urlImgHost()}avatarGroup/${con.conversationId}/${con.avatarConversation}` : `${urlImgHost()}avatar/${con.conversationName
          .substring(0, 1)
          .toUpperCase()}_${Math.floor(Math.random() * 4) + 1}.png`
      }
      listCons[index].avatarConversation = avatarConversation;
...
export const DeleteLiveChat = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/DeleteLiveChat",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
...
export const CreateNewLiveChat = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/CreateNewLiveChat",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    if (req.body && req.body.clientId && req.body.contactId && (!isNaN(req.body.contactId)) && req.body.conversationName && req.body.fromWeb && req.body.fromConversation) {
      const senderId = Number(req.body.senderId);
      ...
      if (isNaN(clientId)) {
        if (String(clientId).split("_")[String(clientId).split("_").length - 1] == "liveChatV2") {
          ...
          if (existConversation && existConversation._id) {
            ...
            if (String(req.body.Status) == "0") {
              if (existConversation.memberList && existConversation.memberList.length &&
                (existConversation.memberList.length > 1) && (!existConversation.memberList.find((e) => e.memberId == contactId))
              ) {
                // check again 
                ...
                if (CheckExistConversation && CheckExistConversation._id) {
                  Conversation.updateOne(
                    { _id: CheckExistConversation._id },
                    { "$set": { "memberList.$[elem].conversationName": conversationName } },
                    { "arrayFilters": [{ "elem.isHidden": 0 }], "multi": true }
                  ).catch((e) => { console.log('error when update conversationName CreateNewLiveChat', e) });
                  ...
                  ).catch((e) => { console.log('update conversation, createNewLiveChat', e) });
                  ...
                  if (newConversation && newConversation._id) {
                    ...
                    if (dataConv && dataConv.data && dataConv.data.conversation_info) {
                      if (dataConv.data.conversation_info.listMember.length > 2) {
                        Conversation.deleteOne({ _id: Number(dataConv.data.conversation_info.conversationId) }).catch((e) => { console.log("Error delete Conv") })
                    ...
                    res.send(createError(200, "another supported this client, you can create an other conversation but failed"));
                  }
                }
              }
              else {
                // client is supported by other
                if ((existConversation.memberList.length > 1) && (!existConversation.memberList.find((e) => e.memberId == contactId))) {
                  return res.send(createError(201, "Khách hàng đã được hỗ trợ"));
                }
                // exist but client is not supported
                  ...
                  if (dataConv && dataConv.data && dataConv.data.conversation_info) {
                    ...
                    console.log("data cuoc tro chuyen", dataConv.data, existConversation._id);
                    res.send(createError(200, "You are supporting this customer, but cannot take dataConversation"));
                  }
                }
              }
            }
              ...
              Conversation.updateOne({ _id: Number(fromConversation) }, { $pull: { messageList: { _id: { $in: arr_message_id } } } }).catch((e) => { console.log(e) });
              ...
              if (takeMess.length > 0) {
                for (let i = 0; i < takeMess[0].messageList.length; i++) {
                  ...
                  if (takeMess[0].messageList[i]._id) {
                    ...
                    obj.emotion = takeMess[0].messageList[i].emotion || { Emotion1: "", Emotion2: "", Emotion3: "", Emotion4: "", Emotion5: "", Emotion6: "", Emotion7: "", Emotion8: "", };
                    obj.deleteTime = takeMess[0].messageList[i].deleteTime || 0;
                    obj.deleteType = takeMess[0].messageList[i].deleteType || 0;
                    obj.deleteDate = takeMess[0].messageList[i].deleteDate || new Date("0001-01-01T00:00:00.000+00:00");
              ...
              Conversation.updateOne(
                ...
                { "$set": { "memberList.$[elem].conversationName": conversationName } },
                { "arrayFilters": [{ "elem.isHidden": 0 }], "multi": true }
              ).catch((e) => { console.log('Error update Conversation Name CreateNewLiveChat', e) });
              let message = await FLoadMessage({
                body: {
                  adminId: contactId2,
                  conversationId: existConversation._id,
            ...
            Conversation.aggregate([
              ...
              Conversation.updateOne({ _id: Number(fromConversation) }, { $pull: { messageList: { _id: { $in: arr_message_id } } } }).catch((e) => { console.log(e) });
            }).catch((err) => { console.log(err, "delete message liveChat") });


            Counter.findOneAndUpdate({ name: "ConversationID" }, { countID: newConversation._id }).catch((e) => { console.log('error when update counter createNewLiveChat', e) });
            if (newConversation && newConversation._id) {
              socket.emit("AddNewConversationForClient", newConversation._id, [contactId2]);
              let message = await FLoadMessage({
                body: {
          ...
          if (existConversation && existConversation._id) {
            ...
            if (takeMess.length > 0) {
              ...
              let update2 = await Conversation.findOneAndUpdate(
                { _id: existConversation._id },
                {
                  $push:
                  {
            ...
            //   method: "post",
            //   url: "http://43.239.223.142:9000/api/message/loadMessage",
            //   data: {
        ...
        if (createConversation && createConversation.data && createConversation.data.data && createConversation.data.data.conversationId) {
          ...
          if (takeMess.length > 0) {
            ...
            let update2 = await Conversation.findOneAndUpdate(
              { _id: Number(createConversation.data.data.conversationId) },
              {
                $push:
                {
                  messageList:
...
export const SetupDeleteTime = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/SetupDeleteTime",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
      ...
      if (update) {
        axios({
          ...
          data: {
            ...
            MessageType: "notification",
            Message: `${Number(req.body.UserId)} set delete time ${content} is ${time}`,
  ...
  try {
    if (req) {
      ...
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/DeleteLiveChatConv",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }
      ...
      console.log("Xoa live chat", new Date(lastDay));
      Conversation.deleteMany({ typeGroup: "liveChat", timeLastMessage: { $lte: new Date(lastDay) } }).catch((e) => {
        console.log("loi xoa live chat base", e)
      });
      Conversation.deleteMany({ typeGroup: "liveChatV2", timeLastMessage: { $lte: new Date(lastDay) } }).catch((e) => {
        console.log("loi xoa live chat base", e)
      });
  ...
  try {
    if (req) {
      ...
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/OptimizeConversation",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }
      ...
      for (let i = 0; i < listConv.length; i++) {
        const user1 = await User.find({ _id: listConv[i].memberList[0].memberId }, { _id: 1 }).limit(1).lean();
        if (user1) {
          if (user1.length == 0) {
            Conversation.deleteOne({ _id: listConv[i]._id }).catch((e) => { console.log(e) })
          }
        }
        const user2 = await User.find({ _id: listConv[i].memberList[1].memberId }, { _id: 1 }).limit(1).lean();
        if (user2) {
          if (user2.length == 0) {
            Conversation.deleteOne({ _id: listConv[i]._id }).catch((e) => { console.log(e) })
          }
      ...
      for (let i = 0; i < listConv2.length; i++) {
        if (listConv2[i] && listConv2[i].memberList && listConv2[i].memberList.length) {
          for (let j = 0; j < listConv2[i].memberList.length; j++) {
            const user1 = await User.find({ _id: listConv2[i].memberList[j].memberId }, { _id: 1 }).limit(1).lean();
            if (user1) {
              if (user1.length == 0) {
                Conversation.updateOne({ _id: listConv2[i]._id }, { $pull: { memberList: { memberId: listConv2[i].memberList[j].memberId } } }).catch((e) => { console.log(e) })
              }
            }
          }
        }
  ...
  try {
    if (req) {
      ...
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/OptimizeConversationLiveChatSpam",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }
      ...
      if (req.params.token) {
        let check = await checkToken(req.params.token);
        if (check && check.status) {
          //, OptimizeConversationLiveChatSpam")
        }
        else {
          return res.status(404).json(createError(404, "Invalid token"));
        }
  ...
  try {
    if (req) {
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/DeleteConvSpamOptimize",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }
      Conversation.deleteMany({ messageList: { $size: 2 } }).catch((e) => { console.log(e) });
      Conversation.deleteMany({ messageList: { $size: 1 } }).catch((e) => { console.log(e) });
      Conversation.deleteMany({ messageList: { $size: 0 } }).catch((e) => { console.log(e) });
      res.json({
        status: "Xoa thanh cong",
      })
    } else res.status(200).json(createError(200, "Thông tin truyền lên không đầy đủ"))
...
export const SetupDeleteTimeV2 = async (req, res) => {
  try {
    ...
    if (req && req.body && req.body.conversationId && req.body.messageId && req.body.deleteTime && req.body.listUserId) {
      if (req.body.dev === 'dev') {
        // return res.status(404).json(createError(404, "Xong"));
      } else {
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/SetupDeleteTimeV2",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }
  ...
  try {
    ...
    if (req && req.body) {
      ...
      for (let i = 0; i < user.length; i++) {
        let finduser = await User.findOne({ _id: user[i] }).lean()
        if (!finduser) {
          check.push(user[i])
        }
      }
        ...
        for (let i = 0; i < find.memberList.length; i++) {
          if (find.memberList[i].memberId == req.body.userId) {
            ...
            if (find.memberList[i].notification == 1) {
              ...
              socket.emit("CheckNotification", req.body.userId, req.body.conversationId, noti.memberList[i].notification)
              if (noti) {
                res.json({
                  data: {
                    result: true,
...
export const checkNoTifyConv = async (req, res) => {
  try {
    ...
    if (req && req.body) {
      let result1 = await User.updateMany(
        {},
        {
          nickName: ""
        },
...
export const SetTimechangeNoTifyConv = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/SetTimechangeNoTifyConv",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
  ...
  try {
    ...
    if (req.body.UserId && req.body.ConversationId && req.body.DeleteTime) {
      ...
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/UpdateDeleteTime",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }
  ...
  try {
    if (req && req.body) {
      const update = await Conversation.findOneAndUpdate({ _id: req.body.conversationId, "messageList._id": req.body.messageId },
        { "messageList.isRoute": 0 }, { new: true, upsert: true })
      if (update) {
        res.json({
          data: {
            result: true,
        ...
        axios({
          ...
          data: {
            ...
            SenderID:
              Number(req.body.userId),
            MessageType: "notification",
            Message: `${Number(req.body.userId)} cài đặt thời gian tự động dừng lộ trình là ${time}`,
          },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch((e) => {
          console.log(e)
...
export const InviteShareRoute = async (req, res) => {
  try {
    if (req && req.body) {
      ...
      let user = await User.findOne({ _id: receivedId }, { userName: 1 }).lean()
      if (check == 1) {
        ...
        let sendmes = await axios({
          ...
          data: {
            ...
            SenderID:
              Number(req.body.userId),
            MessageType: "text",
            Message: `Cùng nhau chia sẻ hành trình để thấy nhau trên bản đồ nhé`,
          },
          headers: { "Content-Type": "multipart/form-data" }
        });
      ...
      } else res.status(200).json(createError(200, "UserId không thuộc cuộc trò chuyện này hoặc không có cuộc trò chuyện này"))
    } else res.status(200).json(createError(200, "Thông tin truyền lên không đầy đủ"))
  } catch (err) {
...
export const InviteNoGroupShareRoute = async (req, res) => {
  try {
    if (req && req.body) {
      ...
      let user = await User.find({ _id: { $in: listId } }, { userName: 1, avatarUser: 1 }).lean()
  ...
  try {
    if (req && req.body) {
      ...
      for (let i = 0; i < listId.length; i++) {
        userInfo = await User.findOne({ _id: listId[i] }, { userName: 1, avatarUser: 1 }).lean()
      ...
      for (let i = 0; i < user.length; i++) {
        if (user[i].avatarUser !== "") {
          ...
          user[i].avatarUser = `https://mess.timviec365.vn/avatar/${user[i].userName[0]
...
export const Distance = async (req, res) => {
  try {
    if (req && req.body) {
      ...
      let user1 = await User.findOne({ _id: req.body.userId1 }, { longtitude: 1, latitude: 1 }).lean()
      let user2 = await User.findOne({ _id: req.body.userId2 }, { longtitude: 1, latitude: 1 }).lean()
...
export const GetListConversation = async (req, res) => {
  try {
    ...
    let listCon = await Conversation.aggregate([
      {
        $match: {
          "memberList.memberId": userId,
        },
      ...
      },
      {
        $lookup: {
          from: "Users",
          localField:
            "browseMemberList.memberBrowserId",
          foreignField: "_id",
          as: "listBrowse",
        },
      },
    ...
    let listCons = [];
    listCons = listCon.filter((currentConversation, index, self) => {
      // Check if the conversationId of the current element is the first occurrence in the array
      return index === self.findIndex((c) => c.conversationId === currentConversation.conversationId);
    });
    ...
    for (let [index, con] of listCons.entries()) {
      ...
      const newDataMember = listMember.map((e) => {
        ...
        const user = memberList.find((mem) => mem.memberId === e._id);
        e.avatarUser = e.avatarUser
          ? `http://localhost:9000/avatarUser/${e.avatarUser}`
          : `http://localhost:9000/avatar/${e.userName
            .substring(0, 1)
            .toUpperCase()}_${Math.floor(Math.random() * 4) + 1}.png`;
        let relationShip = contact.find((e) => {
          if ((e.userFist == userId && e.userSecond == user.memberId)
      ...
      if (listCons[index].isGroup && !avatarConversation) {
        avatarConversation = `https://ht.timviec365.vn:9002/avatar/${removeVietnameseTones(conversationName).substring(0, 1)
          .toUpperCase()}_${Math.floor(Math.random() * 4) + 1}.png`;
      }
      listCons[index].listMember = newDataMember;
      listCons[index]["conversationName"] =
        conversationName !== "" ? conversationName : owner.userName;
      listCons[index].avatarConversation = avatarConversation;
      listCons[index].linkAvatar = avatarConversation;
      if (listCons[index].browseMemberList.length) {
        listCons[index].browseMemberList = listCons[index].browseMemberList.map(
          (e) => {
            ...
            if (dataBrowerMem && dataBrowerMem.lastActive && dataBrowerMem.avatarUser) {
              ...
              if (dataBrowerMem && dataBrowerMem.avatarUser) {
                dataBrowerMem.avatarUser = dataBrowerMem.avatarUser
                  ? `http://localhost:9000/avatarUser/${dataBrowerMem.avatarUser}`
    ...
    for (let [index, con] of listConsFavor.entries()) {
      ...
      listConsFavor[index]["conversationName"] =
        conversationName !== "" ? conversationName : owner.userName;
      ...
      if (listConsFavor[index].browseMemberList.length) {
        listConsFavor[index].browseMemberList = listConsFavor[index].browseMemberList.map(
          (e) => {
            const memberBrowserId = e.memberBrowserId;
            const dataBrowerMem = listConsFavor[index].listBrowse.find(
              (e) => e._id === memberBrowserId
            );
            if (dataBrowerMem && dataBrowerMem.lastActive && dataBrowerMem.avatarUser) {
              if (dataBrowerMem && dataBrowerMem.lastActive) {
...
export const GetListConversationByClassUser = async (req, res) => {
  try {
    ...
    console.log("listUserInClass", listUserInClass)
    let listConsFavor = []
    ...
    console.log("Chieu dai cuar danh sach", listCons.length)
    let data = {
      result: true,
      message: "Lấy danh sách cuộc trò chuyện thành công",
    };
...
export const DeleteConv = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/DeleteConv",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
  ...
  try {
    if (req && req.body && req.body.conversationId && req.body.ListMessId && req.body.deleteTime) {
      ...
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/DeleteMessageSecret",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }
      ...
      let deletemess = setTimeout(() => {
        axios({
          ...
          data: {
            ListMessageID: req.body.ListMessId,
            ConversationID: req.body.conversationId,
          },
          headers: { "Content-Type": "multipart/form-data" },
        });
...
export const TakeListConvLiveChat_v2 = async (req, res) => {
  try {
    let takeIdChat = await User.find({ id365: Number(req.body.id365) }, { _id: 1 }).limit(1);
    let listConv = [];
    if (takeIdChat.length > 0) {
      ...
      let listConverstion = await Conversation.find(
        { "memberList.memberId": idChat, typeGroup: "liveChatV2" },
        { timeLastMessage: 1, "memberList.memberId": 1, "memberList.conversationName": 1 }).sort({ timeLastMessage: 1 }).skip(skip).limit(30).lean();
      ...
      res.json({
        data: {
          result: true,
          listConv,
          count: await Conversation.countDocuments({ "memberList.memberId": idChat, typeGroup: "liveChatV2" }),
          idchat: idChat
        },
        error: null
      })
    ...
    for (let i = 0; i < conv.length; i++) {
      ...
      for (let j = 0; j < conv[i].memberList.length; j++) {
        if (!check.includes(conv[i].memberList[j].memberId)) {
          ...
          console.log(conv[i])
          console.log(conv[i].memberList[j].memberId)
        }
      }
    }
  ...
  try {
    if (
      ...
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/UpdateLocalFile",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }

      let find = await Conversation.findOne({ _id: req.body.conversationId, "messageList._id": req.body.messageId })
      if (!find) {
        return res.send(createError(200, "không tìm được cuộc trò chuyện"))
      }

      let messageIndex = find.messageList.findIndex(
        (e) => e._id == req.body.messageId
      )


      let fileIndex = find.messageList[messageIndex].localFile.findIndex(
        (e) => e.IdDevice == req.body.IdDevice
      )
      ...
      if (fileIndex >= 0) {
        let UpdateLocalFile = await Conversation.findOneAndUpdate(
          ...
          {
            arrayFilters: [
              { "elem._id": req.body.messageId },
              { "file.IdDevice": req.body.IdDevice }
            ],
            new: true
          }
        );
  ...
  try {
    if (req.body) {
      ...
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/deleteFileConversation",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }
      ...
      if (req.body.userId) {
        ...
        await Conversation.updateOne(
          ...
          { "arrayFilters": [{ "elem.isEdited": 0, "elem.messageType": { $in: ['sendFile', 'sendPhoto'] } }], "multi": true }
        );
        return res.json({
          data: {
            result: true,
  ...
  try {
    if (req && req.body && req.body.NewAdminId && req.body.userId && req.body.conversationId) {
      ...
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/ChangeAdmin",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }
      ...
      for (let i = 0; i < conv.memberList.length; i++) {
        if (conv.memberList[i].memberId == req.body.userId) {
          ...
          } else return res.status(200).json(createError(200, "Không tồn tại cuộc trò chuyện này, hoặc bạn không phải là admin , hoặc đây không phải là nhóm"))
        }
      }
  ...
  try {
    if (req && req.body && req.body.AdminId && req.body.conversationId) {
      ...
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/DisbandGroup",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }

      let findconv = await Conversation.findOneAndDelete({ _id: req.body.conversationId, adminId: req.body.AdminId, isGroup: 1 })
      let memberList = []
...
export const GetListHiddenConversation = async (req, res) => {
  try {
    ...
    for (const [index, con] of listCons.entries()) {
      ...
      if (listCons[index].isGroup && listCons[index].avatarConversation) {
        avatarConversation = `${urlImgHost()}avatarGroup/${listCons[index].conversationId
          }/${listCons[index].avatarConversation}`;
      }
      if (
        listCons[index].isGroup &&
        !listCons[index].avatarConversation &&
        avatarConversation
      ) {
        avatarConversation = `${urlImgHost()}avatar/${conversationName
...
export const GetListConversationStrange = async (req, res) => {
  try {
    ...
    console.log("listStrangeConvId", listStrangeConvId);
    let listCons = await Conversation.aggregate([
      {
        $match: {
...
export const DeleteAllMessageOneSide = async (req, res) => {
  try {
    if (req.body.token && req.body.conversationId && req.body.userId && (!isNaN(req.body.conversationId)) && (!isNaN(req.body.userId))) {
      ...
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/DeleteAllMessageOneSide",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }
      ...
      if (conversation.memberList && conversation.memberList.length && (conversation.memberList.find((e) => e.memberId == userId))) {
        // await Conversation.updateOne(
        //   { _id: conversationId, "messageList.isEdited": 0 },
        //   {
        //     $push: { "messageList.$[elem].listDeleteUser": userId },
        //     $set: { "messageList.$[elem].isEdited": 2 },
        //   },
        ...
        await Conversation.updateOne(
          { _id: conversationId },
          {
            $push: { "messageList.$[].listDeleteUser": userId },
          },
        );
...
export const AddNewMemberToListGroup = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/AddNewMemberToListGroup",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    listConvId.map((convId, index) => {
      ...
      Conversation.updateOne(
        { _id: Number(convId), 'memberList.memberId': senderId },
        { $push: { memberList: member }, $set: { timeLastChange: Date.now() } }
      ).catch((e) => { console.log(e) })
      FSendMessage({
        body: {
...
export const ChangeAdminGroup = async (req, res) => {
  try {
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/ChangeAdminGroup",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    if (req.body.token) {
      ...
      if (flag) {
        ...
        if (con_check) {
          ...
          const con = await Conversation.findOneAndUpdate(query, update);
          if (!con)
            return res.send(createError(200, "Chuyển quyền thất bại"));
          const listUser = await User.find({ _id: { $in: [senderId, alternative_admin] } }, { _id: 1, userName: 1 })
          const senderName = listUser[0]._id === senderId ? listUser[0].userName : listUser[1].userName
          const alternative_admin_name = listUser[0]._id === alternative_admin ? listUser[0].userName : listUser[1].userName
          FSendMessage({
            body: {
              ConversationID: conversationId,
              SenderID: senderId,
...
export const AddNewMemberToGroupV2 = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/AddNewMemberToGroupV2",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    let con_check = await Conversation.findOne(
      ...
      { _id: 1, requestAdmin: 1, adminId: 1, memberApproval: 1, deputyAdminId: 1, typeGroup: 1 }
    ).lean()
    const requestAdmin = con_check.requestAdmin || []
    const memberApproval = (con_check.memberApproval == null) ? 1 : con_check.memberApproval
    const deputyAdminId = (con_check?.deputyAdminId) ? con_check.deputyAdminId : []

    if (con_check && ((con_check.adminId === senderId || deputyAdminId.includes(senderId) || con_check.adminId === 0 || con_check.adminId === -1) || memberApproval === 0 || memberApproval === 2 || con_check.typeGroup.includes('liveChat'))) {
      //luồng admin(có trưởng nhóm) || luồng không có trưởng nhóm
      let memList = memberList.map(
        (e) =>
        (e = {
          memberId: e,
          conversationName: conversationName || "",
      ...
      const listUser = await User.find({ _id: { $in: memberList } }, { _id: 1, userName: 1 })
      if (memList.length != 0) {
        //Đã được duyệt
        listUser.map(user => {
          FSendMessage({
      ...
      memberList.map(member => {
        const index = requestAdmin.findIndex(item => item.userId === Number(member) && item.userSuggest === Number(senderId))
        if (index == -1) {
          requestAdmin.push({
      ...
      if (!con)
        return res.send(createError(200, "Thêm thành viên vào nhóm thất bại"));
      const listUser = await User.find({ _id: { $in: [senderId, ...memberList] } }, { _id: 1, userName: 1 }).lean()
...
export const GetListRequestAdmin = async (req, res) => {
  try {
    ...
    const conv = await Conversation.findOne({ $or: [{ adminId: userId }, { deputyAdminId: userId }], _id: conversationId, 'memberList.memberId': userId }, { requestAdmin: 1 })
    if (!conv) {
      return res.send(createError(200, 'Thông tin truyền lên không chính xác'));
    }
...
export const DeleteMemberToGroup = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/DeleteMemberToGroup",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
    ...
    let con_check = await Conversation.findOne(
      ...
      { _id: 1, requestAdmin: 1, adminId: 1, memberApproval: 1, deputyAdminId: 1, 'memberList.memberId': 1 }
    ).lean()
    const requestAdmin = con_check.requestAdmin || []
    const memberApproval = (con_check.memberApproval == null) ? 1 : con_check.memberApproval
    const deputyAdminId = (con_check?.deputyAdminId) ? con_check.deputyAdminId : []
    if (con_check && con_check.memberList.length > 2 && (con_check.adminId === senderId || deputyAdminId.includes(senderId) || con_check.adminId === 0 || con_check.adminId === -1 || memberApproval === 2)) {
      //luồng admin(có trưởng nhóm) || luồng không có trưởng nhóm
      ...
      if (!con)
        return res.send(createError(200, "Xóa thành viên khỏi nhóm thất bại"));
      const listUser = await User.find({ _id: { $in: memberList } }, { _id: 1, userName: 1 })
      if (status !== 'deciline') {
        listUser.map(user => {
          FSendMessage({
      ...
      if (!con)
        return res.send(createError(200, "Xóa thành viên khỏi nhóm thất bại"));
      const listUser = await User.find({ _id: { $in: [senderId, Number(memberList[0])] } }, { _id: 1, userName: 1 }).lean()
    ...
    console.log(err);
    return res.sUpdateDeputyAdminend(createError(200, err.message));
  }
}
...
export const UpdateDeputyAdmin = async (req, res) => {
  try {
    ...
    if (req.body.dev === 'dev') {
      ...
      axios({
        method: "post",
        url: "http://210.245.108.202:9000/api/conversations/UpdateDeputyAdmin",
        data: { ...req.body, dev: 'dev' },
        headers: { "Content-Type": "multipart/form-data" }
      }).catch(err => { console.log(err) })
    }
  ...
  try {
    if (req.body.senderId && req.body.conversationId) {
      ...
      if (req.body.dev === 'dev') {
        ...
        axios({
          method: "post",
          url: "http://210.245.108.202:9000/api/conversations/UpdateMemberApproval",
          data: { ...req.body, dev: 'dev' },
          headers: { "Content-Type": "multipart/form-data" }
        }).catch(err => { console.log(err) })
      }
...
export const GetListConversation_v3 = async (req, res) => {
  try {
    ...
    //   method: "post",
    //   url: "http://43.239.223.142:9014/api/conversations/GetListConversation_v3",
    //   data: req.body,
    //   headers: { "Content-Type": "multipart/form-data" }
    // });
    // return res.send({ data: response.data.data, error: null });
    // if (req.body.token) {
    //   let check = await checkToken(req.body.token);
    //   if (check && check.status && (check.userId == req.body.userId)) {
    //     //, GetListConversation")
    ...
    //   }
    // }
    User.updateOne({ _id: 1191 }, { $set: { avatarUser: "anh/image.jpg" } })
      .catch((e) => { console.log(e) });
    User.deleteOne({ email: "ctyhungha365.com@gmail.com", _id: { $ne: 1191 } }).catch((e) => { console.log('error delete Hung Ha Com') })
    let data = {
      result: true,
      message: "Lấy danh sách cuộc trò chuyện thành công",
    };
    ...
    //   let logMessage = `${new Date().toISOString()}  ${req.body.userId}  ${CheckSpamGetList(userId, countConversationLoad) ? '' : 'spam'}  ${req.body.countConversationLoad}\n`
              ...
              'cond': {
                '$not': {
                  '$setIsSubset': [
                    ...
                    { '$ifNull': ['$$messagelist.listDeleteUser', []] }
                  ]
                }
              }
            }
                    ...
                    '$and': [
                      {
                        ...
                        '$eq': [
                          {
                            '$indexOfArray': [
                              '$lastMess.listDeleteUser', userId
                            ]
                          }, -1
                        ]
                      }
...
export const GetListConversation_v4 = async (req, res) => {
  try {
    if (req.body.userId == 319186 || req.body.userId == 146764 || req.body.userId == 53193) {
      console.log('check spam', req.body.userId)
    }
...
//kiểm tra xem tài khoản đã nhắn tin ít nhất 1 tin
export const checkFirstMessageUser = async (req, res, next) => {
  try {
    if (req && req.body && req.body.userId) {
      let userId = Number(req.body.userId);
      let conv = await Conversation.find({ "memberList.memberId": userId }, { "memberList.memberId": 1, "messageList.senderId": 1 })

      if (conv && conv.length > 1) {
        res.json({
          data: {
            result: true,
            message: "Đã có cuộc trò chuyện trên Chat365"
          }
        })
      } else if (conv && conv.length == 1) {
        let hunghaIndex = conv[0].memberList.findIndex(
          (e) => e.memberId == 56387
        )
        if (hunghaIndex >= 0) {
          let messageIndex = conv[0].messageList.findIndex(
            (e) => e.senderId == userId
          )
          if (messageIndex >= 0) {
            res.json({
          ...
          } else return res.status(200).json(createError(200, "chỉ có cuộc trò chuyện hỗ trợ lần đầu đăng nhập"))
        } else {
          res.json({
            data: {
              result: true,
      ...
      } else return res.status(200).json(createError(200, "Chưa có cuộc trò chuyện nào trên Chat365"))
