...
const FileDanger = ['.BAT', '.CHM', '.CMD', '.COM', 'CPL', '.EXE', '.HLP', '.HTA', '.JSE', '.LNK', '.MSI', '.PIF', '.REG', '.SCR', '.SCT', '.SHB', '.SHS', '.VB', '.VBE', '.VBS', '.WSC', '.WSF', 'WSH', '.PHP']
//upload file
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        ...
        if (!fs.existsSync(`C:/Chat365/publish/wwwroot/uploadsImageSmall`)) {
            fs.mkdirSync(`C:/Chat365/publish/wwwroot/uploadsImageSmall`);
        }
...
export const UploadAvatarGroup = async (req, res, next) => {
    try {
        if (req.file) {
            ...
            if (req.file.mimetype === 'application/octet-stream' || req.file.mimetype === 'image/jpeg' || req.file.mimetype === 'image/jpg' || req.file.mimetype === 'image/png') {
                let conversationId
                if (req.file.originalname.includes('.')) {
                    conversationId = req.file.originalname.slice(req.file.originalname.indexOf('_') + 1, req.file.originalname.indexOf('.'))
                }
                else {
                    conversationId = req.file.originalname.slice(req.file.originalname.indexOf('_') + 1)
                }
                if (!fs.existsSync('C:/Chat365/publish/wwwroot/avatarGroup')) {
                    fs.mkdirSync('C:/Chat365/publish/wwwroot/avatarGroup');
                }
                if (!fs.existsSync(`C:/Chat365/publish/wwwroot/avatarGroup/${String(conversationId)}`)) {
                    fs.mkdirSync(`C:/Chat365/publish/wwwroot/avatarGroup/${String(conversationId)}`);
                }
                ...
                await sharp(req.file.buffer)
                    ...
                    .toFile(`C:/Chat365/publish/wwwroot/avatarGroup/${conversationId}/${avatarConversation}`)
                const fileDelete = fs.readdirSync(`C:/Chat365/publish/wwwroot/avatarGroup/${String(conversationId)}`)[0]
                if (fs.readdirSync(`C:/Chat365/publish/wwwroot/avatarGroup/${String(conversationId)}`).length >= 10) {
                    fs.unlinkSync(`C:/Chat365/publish/wwwroot/avatarGroup/${String(conversationId)}/${fileDelete}`)
                }
...
export const UploadAvatar = async (req, res, next) => {
    if (req.file) {
        ...
        if (req.file.mimetype === 'application/octet-stream' || req.file.mimetype === 'image/jpeg' || req.file.mimetype === 'image/jpg' || req.file.mimetype === 'image/png') {
            let userId
            if (req.file.originalname.includes('.')) {
                userId = req.file.originalname.slice(req.file.originalname.indexOf('_') + 1, req.file.originalname.indexOf('.'))
            }
            else {
            ...
            if (user) {
                ...
                const send1 = () => {
                    if (user.type365 != 0) {
                        let response = axios.post('https://chamcong.24hpay.vn/api_chat365/update_avatar.php', qs.stringify({
        ...
        if (req.files.length > 0) {
            ...
            for (let i = 0; i < files.length; i++) {
                if (req.files[i].filename.toUpperCase().split(".")[req.files[i].filename.toUpperCase().split(".").length - 1].includes('JPEG')) {
                    await sharp(`C:/Chat365/publish/wwwroot/uploads/${files[i].replace(/[ +!@#$%^&*]/g, '')}`)
                        .resize({ fit: sharp.fit.contain, width: 1200, height: 1200 })
                        .toFile(`C:/Chat365/publish/wwwroot/uploadsImageSmall/${files[i]}`)
                }
        ...
        if (user) {
            for (let i = 0; i < user.length; i++) {
                if (fs.existsSync(`C:/Chat365/publish/wwwroot/avatarUser/${String(user[i]._id)}`)) {
                    fs.readdirSync(`C:/Chat365/publish/wwwroot/avatarUser/${String(user[i]._id)}`).forEach((file, index) => {
                        const curPath = path.join(`C:/Chat365/publish/wwwroot/avatarUser/${String(user[i]._id)}`, file);
                        if (fs.readdirSync(`C:/Chat365/publish/wwwroot/avatarUser/${String(user[i]._id)}`).length > 1) {
        ...
        if (conversation) {
            for (let i = 0; i < conversation.length; i++) {
                if (fs.existsSync(`C:/Chat365/publish/wwwroot/avatarGroup/${String(conversation[i]._id)}`)) {
                    fs.readdirSync(`C:/Chat365/publish/wwwroot/avatarGroup/${String(conversation[i]._id)}`).forEach((file, index) => {
                        const curPath = path.join(`C:/Chat365/publish/wwwroot/avatarGroup/${String(conversation[i]._id)}`, file);
                        if (fs.readdirSync(`C:/Chat365/publish/wwwroot/avatarGroup/${String(conversation[i]._id)}`).length > 1) {
                            fs.unlinkSync(curPath);
                        }
                    });
                }
...
export const UploadNewFile = async (req, res, next) => {
    try {
        ...
        let files = []
        for (let i = 0; i < req.files.length; i++) {
            // console.log("Du lieu upload new file",listFile[i].FullName.replace(/\s+/g, ''));
            // listFile[i].FullName = listFile[i].FullName.replace(/\s+/g, '');
            listFile[i].NameDownload = listFile[i].FullName.replace(/[ +!@#$%^&*]/g, '');
            // files.push(fInfoFile(
            //     listFile[i].TypeFile,
            //     listFile[i].FullName.replace(/\s+/g, ''),
            //     listFile[i].sizeFile,
            //     listFile[i].Height,
            //     listFile[i].Width
            // ))
            const name = req.files[i].originalname.slice(req.files[i].originalname.indexOf('.'), req.files[i].originalname.length)
            if (FileDanger.includes(name.toUpperCase())) {
                return res.status(200).json(createError(200, "File được chọn không thể upload"))
            }
            fs.writeFileSync(`C:/Chat365/publish/wwwroot/uploads/${listFile[i].FullName.replace(/[ +!@#$%^&*]/g, '')}`, req.files[i].buffer)
            if (req.files[i].originalname.toUpperCase().includes('JPEG') || req.files[i].originalname.toUpperCase().includes('JPG') || req.files[i].originalname.toUpperCase().includes('PNG')) {
                await sharp(req.files[i].buffer)
                    .resize({ fit: sharp.fit.contain, width: 120, height: 120 })
                    .toFile(`C:/Chat365/publish/wwwroot/uploadsImageSmall/${listFile[i].FullName.replace(/[ +!@#$%^&*]/g, '')}`)
            }
        }
        const index = (await Conversation.aggregate([
            {
                '$match': {
        ...
        const mess = {
            ...
            'Message': '',
            'CreateAt': `${JSON.parse(JSON.stringify(new Date(new Date().setHours(new Date().getHours() + 7)))).replace("Z", "")}+07:00`,
            'IsEdited': 0,
            'DeleteTime': 0,
            'DeleteDate': new Date(),
            'DeleteType': 0,
        ...
        const listMember = [], listDevices = [], listfromWeb = [], isOnline = []
        let currentWeb = ''
        for (let i = 0; i < conv.memberList.length; i++) {
            if ((mess.LiveChat && mess.LiveChat.ClientId) || conv.typeGroup === 'liveChat') {
                if (conv.memberList[i].liveChat && conv.memberList[i].liveChat.clientId) {
                    if (!listDevices.includes(conv.memberList[i].liveChat.clientId)) listDevices.push(conv.memberList[i].liveChat.clientId)
                    if (listMember.includes(conv.memberList[i].memberId)) listMember.splice(listMember.indexOf(conv.memberList[i].memberId), 1)
                    if (conv.memberList[i].memberId === mess.SenderID) currentWeb = conv.memberList[i].liveChat.fromWeb
                }
                else {
                    if (!listMember.includes(conv.memberList[i].memberId)) listMember.push(Number(conv.memberList[i].memberId))
                }
            }
            else {
                if (!listMember.includes(conv.memberList[i].memberId)) listMember.push(Number(conv.memberList[i].memberId))
        ...
        if ((mess.LiveChat && mess.LiveChat.ClientId) || (conv && conv.typeGroup === 'liveChat')) {
            ...
            SendMessageMqtt(MemberList, mess)
...
export const DownloadAvatarGroup = async (req, res, next) => {
    try {
        if (req.params) {
            ...
            const filePath = `C:/Chat365/publish/wwwroot/avatarGroup/${conversation}/${fileName}`
            if (fs.existsSync(filePath)) {
                // res.download(filePath)
                res.send(fs.readFileSync(filePath, { encoding: 'utf8' }))
            }
...
export const DownloadFile = async (req, res, next) => {
    try {
        if (req.params) {
            const filePath = `C:/Users/hotro/OneDrive/Documents/TestNode/public/fileupload/${req.params.fileName}`
            if (fs.existsSync(filePath)) {
                res.download(filePath)
                // res.send(fs.readFileSync(filePath, {encoding: 'utf8'}))
            }
...
export const DownloadSmallFile = async (req, res, next) => {
    try {
        if (req.params) {
            const filePath = `C:/Chat365/publish/wwwroot/uploadsImageSmall/${req.params.filename}`
    ...
    if (req.body.ID) {
        if (req.file) {
            if (req.file.mimetype === 'application/octet-stream' || req.file.mimetype === 'image/jpeg' || req.file.mimetype === 'image/jpg' || req.file.mimetype === 'image/png') {
                ...
                const user = await User.findOneAndUpdate({ _id: userId }, { avatarUser: avatarUser }, { new: true, projection: { avatarUser: 1, email: 1, password: 1, type365: 1, id365: 1, idTimViec: 1 } })
                if (user) {
                    const file = `C:/Chat365/publish/wwwroot/avatarUser/${user._id}/${user.avatarUser}`
                    const link = `https://mess.timviec365.vn/avatarUser/${user._id}/${user.avatarUser}`
                    const send1 = () => {
                        if (user.type365 != 0) {
                            let response = axios.post('https://chamcong.24hpay.vn/api_chat365/update_avatar.php', qs.stringify({
                                'email': `${String(user.email)}`,
                    ...
                    const send2 = () => {
                        if (user.idTimViec != 0) {
                            ...
                            form.append('email', String(user.email))
                            form.append('type', String(user.type365))
                            form.append('pass', String(user.password));
                            form.append('file', fs.createReadStream(file));
                            axios.post('https://timviec365.vn/api_app/update_tt_chat365.php', form, {
...
export const UploadNewAvatarGroup = async (req, res, next) => {
    try {
        if (req.body.conversationId && req.file) {
            if (req.file.originalname.toUpperCase().includes('JPEG') || req.file.originalname.toUpperCase().includes('JPG') || req.file.originalname.toUpperCase().includes('PNG')) {
                const conversationId = Number(req.body.conversationId)
                if (!fs.existsSync('C:/Chat365/publish/wwwroot/avatarGroup')) {
                    fs.mkdirSync('C:/Chat365/publish/wwwroot/avatarGroup');
                }
        ...
        for (let i = 0; i < user.length; i++) {
            ...
            if (user[i].avatarUser !== '' && fs.existsSync(`C:/Chat365/publish/wwwroot/avatarUser/${user[i]._id}`)) {
                if (!fs.existsSync(`C:/Chat365/publish/wwwroot/avatarUser/${user[i]._id}/${user[i].avatarUser}`)) {
                    ...
                    if (count !== 0) {
                        const fileName = fs.readdirSync(`C:/Chat365/publish/wwwroot/avatarUser/${user[i]._id}`)[count - 1]
