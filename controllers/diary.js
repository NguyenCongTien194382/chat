...
//                     fs.writeFileSync(`public/diaryUpload/${req.body.conversationId}/${pathFile}`, req.files[i].buffer)
...
export const createPostDiary = async (req, res, next) => {
    try {
        ...
        if (req.body.content || req.files.length > 0) {
            ...
            for (let i = 0; i < req.files.length; i++) {
                if (req.files[i].originalname.toUpperCase().includes('JPEG') ||
                ...
                else if (req.files[i].originalname.toUpperCase().includes('MP4') ||
                    ...
                    arr.pop()
                    const thumbnail = `${arr.join('.')}.jpg`
                    ffmpeg(`public/diaryUpload/${req.body.conversationId}/${pathFile}`)
                        .screenshots({
                            count: 1,
                            timemarks: ['00:00:01'],
                            folder: `public/diaryUpload/${req.body.conversationId}`,
                            filename: thumbnail,
                        }).on('end', () => {
            ...
            if (!err) {
                ...
                if (saveddiary) {
                    ...
                    for (let i = 0; i < saveddiary.fileList.length; i++) {
                        saveddiary.fileList[i].pathFile = `${urlImgHost()}Testnode/public/diaryUpload/${saveddiary.conversationId}/${saveddiary.fileList[i].pathFile}`
                        if (saveddiary.fileList[i].thumbnail !== '') {
                            saveddiary.fileList[i].thumbnail = `${urlImgHost()}Testnode/public/diaryUpload/${saveddiary.conversationId}/${saveddiary.fileList[i].thumbnail}`
                        }
                    }
                    const listTag = []
                    for (let i = 0; i < formData.listTag.length; i++) {
                        const user = await User.findOne({ _id: Number(formData.listTag[i]) }, { userName: 1, avatarUser: 1 })
                        if (user.avatarUser !== '') {
                    ...
                    // socket.emit("CreatePostDiary", saveddiary, listUserId);
                    const message = `${saveddiary.userNameSender} đã vừa đăng 1 bài viết`
                    // socket.emit("post", saveddiary, message, listUserId);
                    saveddiary._doc.totalEmotion = saveddiary.emotion.split('/').length - 1
                    //gửi thông báo tới danh sách người được tag
                    // for (let i = 0; i < formData.listTag.length; i++) {
                    //     await axios({
                    //         method: "post",
                ...
                if (diary.length > 0) {
                    ...
                    for (let i = 0; i < diary.length; i++) {
                        ...
                        for (let j = 0; j < diary[i].listTag.length; j++) {
                            const user = await User.findOne({ _id: Number(diary[i].listTag[j]) }, { userName: 1, avatarUser: 1 })
                            if (user.avatarUser !== '') {
                                user.avatarUser = `${urlImgHost()}avatarUser/${user._id}/${user.avatarUser}`
                            }
                            else {
                        ...
                        if (diary[i].avatarUserSender !== '') {
                            diary[i].avatarUserSender = `${urlImgHost()}avatarUser/${diary[i].userSender}/${diary[i].avatarUserSender}`
                        }
                        else {
                            diary[i].avatarUserSender = `${urlImgHost()}avatar/${diary[i].userNameSender[0]}_${Math.floor(Math.random() * 4) + 1}.png`
                        }
                        ...
                        for (let j = 0; j < diary[i].commentList.length; j++) {
                            if (diary[i].commentList[j].commentAvatar !== '') {
                                ...
                                diary[i].commentList[j].commentAvatar = `${urlImgHost()}avatar/${diary[i].commentList[j].commentName[0]}_${Math.floor(Math.random() * 4) + 1}.png`
                            }
                        }

                        for (let j = 0; j < diary[i].fileList.length; j++) {
                            diary[i].fileList[j].pathFile = `${urlImgHost()}Testnode/public/diaryUpload/${diary[i].conversationId}/${diary[i].fileList[j].pathFile}`
                            if (diary[i].fileList[j].thumbnail !== '') {
                                diary[i].fileList[j].thumbnail = `${urlImgHost()}Testnode/public/diaryUpload/${diary[i].conversationId}/${diary[i].fileList[j].thumbnail}`
                            }
                        }
                        ...
                        for (let j = 0; j < ar.length; j++) {
                            const user = await User.findOne({ _id: Number(ar[j]) }, { userName: 1, avatarUser: 1 })
                            if (user.avatarUser !== '') {
                                user.avatarUser = `${urlImgHost()}avatarUser/${user._id}/${user.avatarUser}`
                            }
                            else {
...
//                         postDiary.avatarUserSender = `/avatarUser/${postDiary.userSender}/${postDiary.avatarUserSender}`
//                     }
//                     else {
//                         postDiary.avatarUserSender = `/avatar/${postDiary.userNameSender[0]}_${Math.floor(Math.random() * 4) + 1}.png`
//                     }
//                     for (let i = 0; i < postDiary.commentList.length; i++) {
//                         if (postDiary.commentList[i].commentAvatar !== '') {
//                             postDiary.commentList[i].commentAvatar = `/avatarUser/${postDiary.commentList[i].commentatorId}/${postDiary.commentList[i].commentAvatar}`
//                         }
//                         else {
//                             postDiary.commentList[i].commentAvatar = `/avatar/${postDiary.commentList[i].commentName[0]}_${Math.floor(Math.random() * 4) + 1}.png`
//                         }
//                     }
        ...
        if (req && req.params && req.params._id) {
            ...
            if (postDiary) {
                if (postDiary) {
                    ...
                    postDiary._doc.totalEmotion = postDiary.emotion.split('/').length - 1
                    if (postDiary.avatarUserSender !== '') {
                        postDiary.avatarUserSender = `${urlImgHost()}avatarUser/${postDiary.userSender}/${postDiary.avatarUserSender}`
                    }
                    else {
                        postDiary.avatarUserSender = `${urlImgHost()}avatar/${postDiary.userNameSender[0]}_${Math.floor(Math.random() * 4) + 1}.png`
                    }

                    for (let i = 0; i < postDiary.fileList.length; i++) {
                        postDiary.fileList[i].pathFile = `${urlImgHost()}Testnode/public/diaryUpload/${postDiary.conversationId}/${postDiary.fileList[i].pathFile}`
                        if (postDiary.fileList[i].thumbnail !== '') {
                            postDiary.fileList[i].thumbnail = `${urlImgHost()}Testnode/public/diaryUpload/${postDiary.conversationId}/${postDiary.fileList[i].thumbnail}`
                        }
                    }
                    const listUserEmotion = []
                    ...
                    for (let i = 0; i < postDiary.listTag.length; i++) {
                        const user = await User.findOne({ _id: Number(postDiary.listTag[i]) }, { userName: 1, avatarUser: 1 })
                        if (user.avatarUser !== '') {
                            user.avatarUser = `${urlImgHost()}avatarUser/${user._id}/${user.avatarUser}`
                        }
                        else {
        ...
        if (!err) {
            ...
            if (diary) {
                ...
                for (let i = 0; i < diary.fileList.length; i++) {
                    diary.fileList[i].pathFile = `${urlImgHost()}Testnode/public/diaryUpload/${diary.conversationId}/${diary.fileList[i].pathFile}`
                    if (diary.fileList[i].thumbnail !== '') {
                        diary.fileList[i].thumbnail = `${urlImgHost()}Testnode/public/diaryUpload/${diary.conversationId}/${diary.fileList[i].thumbnail}`
                    }
                }

                const listTag = []
                for (let i = 0; i < diary.listTag.length; i++) {
                    const user = await User.findOne({ _id: Number(diary.listTag[i]) }, { userName: 1, avatarUser: 1 })
                    if (user.avatarUser !== '') {
                        user.avatarUser = `${urlImgHost()}avatarUser/${user._id}/${user.avatarUser}`
                    }
                    else {
            ...
            if (diary) {
                if (diary.length > 0) {
                    ...
                    for (let i = 0; i < diary.length; i++) {
                        ...
                        for (let j = 0; j < diary[i].commentList.length; j++) {
                            if (diary[i].commentList[j].commentAvatar !== '') {
                                diary[i].commentList[j].commentAvatar = `${urlImgHost()}avatarUser/${diary[i].commentList[j].commentatorId}/${diary[i].commentList[j].commentAvatar}`
                            }
                            else {
                                diary[i].commentList[j].commentAvatar = `${urlImgHost()}avatar/${diary[i].commentList[j].commentName[0]}_${Math.floor(Math.random() * 4) + 1}.png`
                            }
            ...
            if (commentList.length > 0) {
                for (var i = 0; i < commentList.length; i++) {
                    ...
                    if (commentList[i].commentAvatar !== '') {
                        commentList[i].commentAvatar = `${urlImgHost()}avatarUser/${commentList[i].commentatorId}/${commentList[i].commentAvatar}`
                    }
                    else {
                        commentList.commentAvatar = `${urlImgHost()}avatar/${commentList[i].commentName[0]}_${Math.floor(Math.random() * 4) + 1}.png`
                    }
                }
                res.json({
                    data: {
            ...
            if (personal) {
                res.json({
                    data: {
                        ...
                        background: `${urlImgHost()}Testnode/public/personalBackgroundImg/${personal.backgroundImage[0].pathFile}`
                    },
                    error: null
                })
            }
